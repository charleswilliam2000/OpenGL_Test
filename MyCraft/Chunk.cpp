...
)
{
	...
	auto getChunkSolidThreshold = [&](float_VEC chunkOffset) -> double {
		const double normalizedChunkY = std::tanh((chunkOffset.y / static_cast<double>(CONSTANTS::Dimension_1DSize) / 3.0)); // Normalize to [-1, 1]
	...
	for (size_t z = 0; z < CONSTANTS::Dimension_1DSize; z++) {
		for (size_t x = 0; x < CONSTANTS::Dimension_1DSize; x++) {

			auto i = z * CONSTANTS::Dimension_1DSize + x;
			const auto columnTotalHeight = heightmap[i];
			const auto columnHeight = (columnTotalHeight / 16u == 0) ? columnTotalHeight : CONSTANTS::Dimension_1DSize;
...
void ChunkMesh::addFace(const uint8_VEC& blockWorldPos, const PackedFaceData& faceData, const BLOCK_ID& type, uint32_t& vertexOffset) {
...
)
{
	...
	if (x == chunkMin.x) { // West
		if (!getNeighborChunkBlock(FACES::WEST, { x, y, z })) {
			addFace({ x, y, z }, PACKED_FACE_DATA[static_cast<int>(FACES::WEST)], blockType, vertexOffset);
		}
	}
	...
	if (y == chunkMin.y) { // Bottom
		if (!getNeighborChunkBlock(FACES::BOTTOM, { x, y, z })) {
			addFace({ x, y, z }, PACKED_FACE_DATA[static_cast<int>(FACES::BOTTOM)], blockType, vertexOffset);
	...
	if (z == chunkMin.z) { // North
		if (!getNeighborChunkBlock(FACES::NORTH, { x, y, z })) {
			addFace({ x, y, z }, PACKED_FACE_DATA[static_cast<int>(FACES::NORTH)], blockType, vertexOffset);
	...
	if (x == chunkMax.x) { // East
		if (!getNeighborChunkBlock(FACES::EAST, { x, y, z })) {
			addFace({ x, y, z }, PACKED_FACE_DATA[static_cast<int>(FACES::EAST)], blockType, vertexOffset);
		}
	}
	else if (!worldChunk.isBlockSolid(x + 1, y, z)) {
		addFace({ x, y, z }, PACKED_FACE_DATA[static_cast<int>(FACES::EAST)], blockType, vertexOffset);
	...
	if (y == chunkMax.y) { // Top
		if (!getNeighborChunkBlock(FACES::TOP, { x, y, z })) {
			addFace({ x, y, z }, PACKED_FACE_DATA[static_cast<int>(FACES::TOP)], blockType, vertexOffset);
		}
	}
	else if (!worldChunk.isBlockSolid(x, y + 1, z)) {
		addFace({ x, y, z }, PACKED_FACE_DATA[static_cast<int>(FACES::TOP)], blockType, vertexOffset);
	...
	if (z == chunkMax.y) { // South
		if (!getNeighborChunkBlock(FACES::SOUTH, { x, y, z })) {
			addFace({ x, y, z }, PACKED_FACE_DATA[static_cast<int>(FACES::SOUTH)], blockType, vertexOffset);
		}
	}
	else if (!worldChunk.isBlockSolid(x, y, z + 1)) {
		addFace({ x, y, z }, PACKED_FACE_DATA[static_cast<int>(FACES::SOUTH)], blockType, vertexOffset);
